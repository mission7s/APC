...
//    procedure WndProc(var Message: TMessage); override;
    ...
    procedure WMSetTimelineRange(var Message: TMessage); message WM_SET_TIMELINE_RANGE;
...
uses UnitSEC, UnitEditEvent, UnitSelectStartOnAir,
...
//  CurrentTime := SystemTimeToDateTime(GV_TimeCurrent);//Now;
  CurrentTime := SystemTimeToEventTime(GV_TimeCurrent);
...
//       (NextStartTime > CurrentTime) and
       (CompareEventTime(NextStartTime, CurrentTime) > 0) and
//       (DateTimeToTimecode(NextStartTime - CurrentTime) <= GV_SettingTimeParameter.AutoIncreaseDurationBefore) then
       (EventTimeToTimecode(GetDurEventTime(NextStartTime, CurrentTime)) <= GV_SettingTimeParameter.AutoIncreaseDurationBefore) then
    begin
...
{    PlayedTime := CurrentTime - EventTimeToDateTime(CueSheetCurr^.StartTime);
    ...
    PlayedTime := GetDurEventTime(CueSheetCurr^.StartTime, CurrentTime);
    RemainTime := GetDurEventTime(CurrentTime, GetEventEndTime(CueSheetCurr^.StartTime, CueSheetCurr^.DurationTC));

    PlayedTimeString := TimecodeToString(PlayedTime.T, False);
    RemainTimeString := TimecodeToString(RemainTime.T, False);
  end
  else
  begin
...
{      RemainTime := IncSecond(EventTimeToDateTime(CueSheetNext^.StartTime) - CurrentTime);
      ...
      RemainTime := GetDurEventTime(CurrentTime, CueSheetNext^.StartTime);
...
{    RemainTargetTime := IncSecond(EventTimeToDateTime(CueSheetTarget^.StartTime) - CurrentTime);
    ...
    RemainTargetTime := GetDurEventTime(CurrentTime, CueSheetTarget^.StartTime);
    RemainTargetTimeString := TimecodeToString(RemainTargetTime.T);
  end
  else
  begin
...
procedure TfrmChannel.WMBeginUpdate(var Message: TMessage);
begin
...
procedure TfrmChannel.WMEndUpdate(var Message: TMessage);
begin
...
procedure TfrmChannel.WMSetOnAir(var Message: TMessage);
var
  ChannelID: Word;
  IsOnAir: Boolean;
begin
...
procedure TfrmChannel.WMSetTimelineRange(var Message: TMessage);
begin
  UpdatePlayListTimeLineRange;
end;

procedure TfrmChannel.WMInsertCueWheet(var Message: TMessage);
var
  Index: Integer;
  Item: PCueSheetItem;
begin
...
procedure TfrmChannel.WMUpdateCueWheet(var Message: TMessage);
var
  Index: Integer;
  Item: PCueSheetItem;
begin
...
procedure TfrmChannel.WMDeleteCueWheet(var Message: TMessage);
var
  Index: Integer;
  Item: PCueSheetItem;
begin
...
procedure TfrmChannel.WMClearCueWheet(var Message: TMessage);
var
  ChannelID: Word;
begin
  ChannelID := Message.WParam;
...
//      ARow := GetRealRow;
      ...
      FIsContrctAll := GetNodeState(Arow);

      if (FIsContrctAll) then ContractAll
      else ExpandAll;
    end;
  end;
end;
...
//            DurationTC := EnterTC; }
            ...
            OutTC := GetPlusTimecode(InTC, FrameToTimecode(TimecodeToFrame(DurationTC) - 1));
...
{          SaveTime.D := 0;
        ...
        begin
          ...
          begin
            OutTC := EnterTC;
            SaveDurationTC := DurationTC;
            DurationTC     := GetDurTimecode(InTC, FrameToTimeCode(TimecodeToFrame(OutTC) + 1));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
//        StrPCopy(Item^.MediaId, FileName);
        S := FileName;
...
begin
  ...
  if (frmAllChannels <> nil) then
    PostMessage(frmAllChannels.Handle, WM_SET_ONAIR, ChannelID, NativeInt(AIsOnAir));
...
begin
  ...
  Assert(False, GetChannelLogStr(lsNormal, ChannelID, 'U444'));
    ...
    if (frmAllChannels <> nil) then
      PostMessage(frmAllChannels.Handle, WM_UPDATE_CUESHEET, Result, NativeInt(Item));
  ...
  Assert(False, GetChannelLogStr(lsNormal, ChannelID, 'I555'));
    ...
    if (frmAllChannels <> nil) then
      PostMessage(frmAllChannels.Handle, WM_INSERT_CUESHEET, AIndex, NativeInt(Item));
...
begin
  ...
  begin
    ...
    if (frmAllChannels <> nil) then
      SendMessage(frmAllChannels.Handle, WM_DELETE_CUESHEET, Index, NativeInt(Item));
    ...
    Assert(False, GetChannelLogStr(lsNormal, ChannelID, Format('SECInputCueSheetW delete cuesheet, ID=%s', [EventIDToString(AEventID)])));
  end;
...
begin
  ...
  if (frmAllChannels <> nil) then
    PostMessage(frmAllChannels.Handle, WM_CLEAR_CUESHEET, ChannelID, 0);
...
begin
  ...
  FServerEventIDNext := AEventID;

  Item := GetCueSheetItemByID(AEventID);
  if (Item <> nil) then
    CueSheetNext := Item;

//  Assert(False, GetChannelLogStr(lsNormal, ChannelID, Format('SECSetCueSheetNextW, Next=%p, ID=%s', [CueSheetNext, EventIDToString(AEventID)])));
...
begin
  ...
  FServerEventIDCurr   := GetChannelNullEventID;
  FServerEventIDNext   := GetChannelNullEventID;
  FServerEventIDTarget := GetChannelNullEventID;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ErrorString := Format(SDurationTCGreaterThenMinDuration, ['Duration', TimecodeToString(GV_SettingThresholdTime.MinDuration)]);
          exit;
        end;

        if (ADuration > GV_SettingThresholdTime.MaxDuration) then
        begin
          ErrorString := Format(SDurationTCLessThenMaxDuration, ['Duration', TimecodeToString(GV_SettingThresholdTime.MaxDuration)]);
          exit;
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      CheckTime := GetPlusEventTime(DateTimeToEventTime(Now), TimecodeToEventTime(GV_SettingThresholdTime.EditLockTime));
      if (CompareEventTime(Item^.StartTime, CheckTime) <= 0) then
      begin
        Result := False;
        exit;
      end;
...
begin
  ...
  begin
    ...
    begin
      CheckTime := GetPlusEventTime(DateTimeToEventTime(Now), TimecodeToEventTime(GV_SettingThresholdTime.EditLockTime));
      if (CompareEventTime(AItem^.StartTime, CheckTime) <= 0) then
      begin
        Result := False;
        exit;
      end;
      ...
      begin
        ...
        case Item^.EventMode of
          ...
          begin
            ...
            if (ParentItem <> nil) then
            begin
              Track.InPoint  := TimecodeToFrame(ParentItem^.StartTime.T) + Trunc(ParentItem^.StartTime.D - FTimelineStartDate) * FTimeLineDaysPerFrames;
              Track.OutPoint := Track.InPoint + TimecodeToFrame(GetProgramDurationByItem(Item));
            end;
          end;
          EM_MAIN:
          begin
            Track.InPoint  := TimecodeToFrame(Item^.StartTime.T) + Trunc(Item^.StartTime.D - FTimelineStartDate) * FTimeLineDaysPerFrames;
            Track.OutPoint := Track.InPoint + TimecodeToFrame(Item^.DurationTC);
          end;
          EM_JOIN:
          begin
            ParentItem := GetParentCueSheetItemByItem(Item);
            ...
            begin
              ...
              Track.InPoint  := TimecodeToFrame(SubStartTime.T) + Trunc(SubStartTime.D - FTimelineStartDate) * FTimeLineDaysPerFrames;
              Track.OutPoint := Track.InPoint + TimecodeToFrame(Item^.DurationTC);
            end;
          end;
          EM_SUB:
...
//        SideFrames := Round(FTimelineSpace / (FrameGap / FrameSkip)) - 1;
        SideFrames := Round(FTimelineSpace / (FrameGap / FrameSkip));
...
begin
  ...
  begin
    ...
    DurTC := GetDurTimecode(ADurationTC, ParentItem^.DurationTC);
...
{            if (FChannelOnAir) then
          ...
          begin
            ...
            if (ADurationTC > ParentItem^.DurationTC) then
              Item^.DurationTC := GetMinusTimecode(Item^.DurationTC, DurTC)
            else
              Item^.DurationTC := GetPlusTimecode(Item^.DurationTC, DurTC);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if {(StartTime < Now) or }(DateTimeToTimecode(StartTime - CurrentTime) <= GV_SettingTimeParameter.MediaCheckTime) then
        begin
...
begin
  ...
  Item := GetMainItemByStartTime(Index, IncMilliSecond(Now, TimecodeToMilliSec(GV_SettingTimeParameter.BlankCheckTime)));
  if (Item = nil) then
  begin
//    if (FBlankCheckThread.FWarningDialog = nil) or (not FBlankCheckThread.FWarningDialog.Showing) then
    begin
      ...
      System.Classes.TThread.CreateAnonymousThread(
        procedure
        begin
          PostMessage(Handle, WM_WARNING_DISPLAY_NOT_EXIST_EVENT, 0, LParam(PChar(WarningText)));
        end).Start;
    end;
...
//    if (R <> D_OK) then exit;
    ...
    try
      ...
      if (R <> D_OK) then
      begin
        Assert(False, GetChannelLogStr(lsError, ChannelID, Format('UpdateSECCheck SetOnAir failed, Channel = %d, OnAir = %s', [ChannelID, BoolToStr(ChannelOnAir, True)])));
        exit;
      end
      else
...
//      if (R <> D_OK) then exit;
      ...
      begin
        ...
        if (R <> D_OK) then
        begin
          Assert(False, GetChannelLogStr(lsError, ChannelID, Format('UpdateSECCheck ClearCueSheet failed, Channel = %d', [ChannelID])));
          exit;
        end
        else
        ...
        begin
          ...
          if (R <> D_OK) then
          begin
            Assert(False, GetChannelLogStr(lsError, ChannelID, Format('UpdateSECCheck InputCueSheet failed, Channel = %d, ID = %s', [ChannelID, EventIDToString(FCueSheetList[I]^.EventID)])));
            exit;
          end
          else
        ...
        begin
          ...
          if (R <> D_OK) then
          begin
            Assert(False, GetChannelLogStr(lsError, ChannelID, Format('UpdateSECCheck SetCueSheetNext failed, Channel = %d, ID = %s', [ChannelID, EventIDToString(CueSheetNext^.EventID)])));
            exit;
          end
          else
        ...
        begin
          ...
          if (R <> D_OK) then
          begin
            Assert(False, GetChannelLogStr(lsError, ChannelID, Format('UpdateSECCheck SetCueSheetCurr failed, Channel = %d, ID = %s', [ChannelID, EventIDToString(CueSheetCurr^.EventID)])));
            exit;
          end
          else
        ...
        begin
          ...
          if (R <> D_OK) then
          begin
            Assert(False, GetChannelLogStr(lsError, ChannelID, Format('UpdateSECCheck SetCueSheetTarget failed, Channel = %d, ID = %s', [ChannelID, EventIDToString(CueSheetTarget^.EventID)])));
            exit;
          end
          else
...
//      ShowMessage(DateTimeToStr(EventTimeToDateTime(StartTime)));
    ...
    CheckTime := GetPlusEventTime(DateTimeToEventTime(Now), TimecodeToEventTime(GV_SettingThresholdTime.EditLockTime));
    EndTime   := GetEventEndTime(StartTime, DurTime);
    if (CompareEventTime(EndTime, CheckTime) <= 0) then
    begin
...
//  if (GetChannelOnAirByID(FChannelID)) then
  ...
  try
    ...
    begin
      ...
      if (CurrItem <> nil) then
      begin
        StartOnAirTime := IncMilliSecond(EventTimeToDateTime(CurrItem^.StartTime), TimecodeToMilliSec(GV_SettingThresholdTime.OnAirLockTime));
        ...
        begin
          ...
          begin
            ...
            try
              ...
              ResetStartTime := GetPlusEventTime(DateTimeToEventTime(Now), TimecodeToEventTime(GV_SettingTimeParameter.AutoIncreaseDurationAmount));
...
//              DisplayPlayListTimeLine(CurrIndex);
    ...
    {          if (FCueSheetNext <> nil) then
                ...
                NextItem := GetMainItemByStartTime(CurrIndex, IncMilliSecond(Now, TimecodeToMilliSec(GV_SettingThresholdTime.OnAirLockTime)));
...
begin
  ...
  begin
    NextStartTime := EventTimeToDateTime(CueSheetNext^.StartTime);
    if (DateTimeToTimecode(NextStartTime - Now) <= GV_SettingThresholdTime.HoldLockTime) then
    begin
      MessageBeep(MB_ICONWARNING);
...
begin
  ...
  begin
    ...
    if (DateTimeToTimecode(CurrEndTime - Now) <= GV_SettingThresholdTime.SetNextLockTime) then
    begin
      MessageBeep(MB_ICONWARNING);
...
//            R := DCSGetEventInfo(Source^.Handles[I]^.DCSID, Source^.Handles[I]^.Handle, NextItem^.EventID, StartTime, DurationTC);
            ...
            begin
              ...
              begin
                ...
                CurrItem^.DurationTC := GetMinusTimecode(CurrItem^.DurationTC, EventTimeToTimecode(GetDurEventTime(NextItem^.StartTime, StartTime)));
                ResetChildItems(CurrIndex, SaveDurationTC);
              end;
...
procedure TfrmChannel.BreakCurrentEvent;
var
  Index: Integer;
  Item: PCueSheetItem;
...
begin
  ...
  begin
    NextStartTime := EventTimeToDateTime(CueSheetNext^.StartTime);
    if (DateTimeToTimecode(NextStartTime - Now) <= GV_SettingThresholdTime.EnqueueLockTime) then
    begin
      MessageBeep(MB_ICONWARNING);
...
begin
  ...
  while not Terminated do
  begin
    R := WaitForMultipleObjects(2, @WaitList, False, TimecodeToMilliSec(GV_SettingTimeParameter.MediaCheckInterval));
...
begin
  ...
  while not Terminated do
  begin
    R := WaitForMultipleObjects(2, @WaitList, False, TimecodeToMilliSec(GV_SettingTimeParameter.BlankCheckInterval));
    if (R = (WAIT_OBJECT_0 + 1)) then break;
