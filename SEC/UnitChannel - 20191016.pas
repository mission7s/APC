...
uses
  ...
  UnitCommons, UnitChannelTimeline, UnitDCSDLL, UnitMCCDLL, UnitSECDLL, UnitConsts, Vcl.ComCtrls;
...
begin
  ...
  if (FChannelTimelineForm <> nil) then
  begin
    FChannelTimelineForm.CalcuratePlayListTimeLineRange(AStartDate, AEndDate);
  end;
...
{                if (CueSheetNext <> nil) then
                ...
                BaseStartTime := DateTimeToEventTime(BaseDateTime);
                BaseStartTime := GetMinusEventTime(BaseStartTime, TimecodeToEventTime(GV_SettingTresholdTime.EditLockTime));
                Assert(False, GetChannelLogStr(lsNormal, ChannelID, Format('Now time is base time. Base start time = %s',
                                                                             [EventTimeToString(BaseStartTime)])));
...
//      OnAirInputEvents(InputIndex, 1);
      ...
      PostMessage(FChannelForm.Handle, WM_INPUT_NEXT_EVENT, InputIndex, 0);
    end;
