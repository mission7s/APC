...
uses
  ...
  UnitCommons, UnitChannelTimeline, UnitDCSDLL, UnitMCCDLL, UnitConsts, Vcl.ComCtrls;
...
//    FFirstStartTime: TEventTime;
    ...
    FChannelTimelineForm: TfrmChannelTimeline;
...
//    procedure DisplayPlayListGrid(AIndex: Integer; ACount: Integer = 0); overload;
  ...
  public
    { Public declarations }
    constructor Create(AOwner: TComponent; AChannelID: Word; AChannelTimeline: TfrmChannelTimeline; ACombine: Boolean; ALeft, ATop, AWidth, AHeight: Integer); overload;
...
//    procedure DeletePlayList(AFileName: String);
    ...
    procedure MCCInputCueSheets(AChannelID: Word; AIndex: Integer); overload;
    procedure MCCInputCueSheets(AIndex: Integer; AItem: PCueSheetItem); overload;
    procedure MCCDeleteCueSheets(AChannelID: Word; ARemoveIndexList: TList<Integer>); overload;
  ...
  case Message.Msg of
    ...
    begin
      ...
      begin
        ...
        if (CueSheetNext^.EventMode = EM_MAIN) and
...
//            DisplayPlayListGrid(CurrIndex);
            ...
            if (ChannelOnAir) then
              MCCInputCueSheets(ChannelID, CurrIndex);
          end
          else
          begin
...
//            DisplayPlayListGrid(NextIndex);
            ...
            if (ChannelOnAir) then
              MCCInputCueSheets(ChannelID, NextIndex);
          end;
        end;
...
//          FChannelTimelineForm.SetFrameNumber(wmtlPlaylist.FrameNumber);
      ...
      if (FChannelTimelineForm <> nil) then
      begin
        FChannelTimelineForm.SetChannelTime(PlayedTimeString, RemainTimeString, NextStartTimeString, wmtlPlaylist.FrameNumber);
      end;
    end;
...
//            if (R >= FixedRows) or (R <= RowCount - 1) then
      ...
      if (FChannelTimelineForm <> nil) then
        FChannelTimelineForm.SetCueSheetCurr(Item);
    end;
...
//            if (R >= FixedRows) or (R <= RowCount - 1) then
      ...
      if (FChannelTimelineForm <> nil) then
        FChannelTimelineForm.SetCueSheetNext(Item);
    end;
...
//      FMediaCheckIntervalTime := 0;
    ...
    WM_UPDATE_MCC_CHECK:
    begin
      UpdateMCCCheck(Message.WParam, Boolean(Message.LParam));
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Item^.MediaStatus = msNotExist) then
        begin
...
constructor TfrmChannel.Create(AOwner: TComponent; AChannelID: Word; AChannelTimeline: TfrmChannelTimeline; ACombine: Boolean; ALeft, ATop, AWidth, AHeight: Integer);
begin
  inherited Create(AOwner, ACombine, ALeft, ATop, AWidth, AHeight);

  FChannelID := AChannelId;

  FChannelTimelineForm := AChannelTimeline;
end;
...
begin
  ...
  begin
    ...
    begin
      if (AValue <> nil) then
        MCCSetCueSheetCurrs(AValue^.EventID)
      else
        MCCSetCueSheetCurrs(GV_NullEventID);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (AValue <> nil) then
        MCCSetCueSheetNexts(AValue^.EventID)
      else
        MCCSetCueSheetNexts(GV_NullEventID);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (AValue <> nil) then
        MCCSetCueSheetTargets(AValue^.EventID)
      else
        MCCSetCueSheetTargets(GV_NullEventID);
    end;
  end;
end;
...
begin
  ...
  if (ChannelOnAir) then
    MCCInputCueSheets(ChannelID, AIndex);
end;
...
//  DisplayPlayListGrid(Index, 1);
  ...
  if (ChannelOnAir) then
    MCCInputCueSheets(ChannelID, Index);
end;
...
begin
  ...
  {        if (Item^.EventMode = EM_MAIN) then
    ...
    if (FChannelTimelineForm <> nil) then
      FChannelTimelineForm.wmtlPlaylist.BeginUpdateCompositions;
...
{          ChannelCueSheet := GetChannelCueSheetByItem(Item);
      ...
      if (ChannelOnAir) then
        MCCDeleteCueSheets(ChannelID, RemoveRowList);
  ...
  //    acgPlaylist.RemoveSelectedRows;
      ...
      if (FChannelTimelineForm <> nil) then
        FChannelTimelineForm.wmtlPlaylist.EndUpdateCompositions;
    end;
  ...
  //  DisplayPlayListGrid(FirstIndex, -ADeleteList.Count);
    ...
    if (ChannelOnAir) then
      MCCInputCueSheets(ChannelID, FirstIndex);
  finally
    Screen.Cursor := crDefault;
  end;
end;
...
//  acgPlaylist.SelectRows(FirstIndex + CNT_CUESHEET_HEADER, IDX_COL_CUESHEET_NO);
    ...
    if (FChannelOnAir) then
      MCCInputCueSheets(ChannelID, SelectIndex);
  finally
    PasteList.Clear;
...
//
//        if (FChannelTimelineForm <> nil) then
//          FChannelTimelineForm.PopulatePlayListTimeLine(CompIndex, Track);
      end;
...
//  if (AIndex < 0) or (AIndex > FCueSheetList.Count - 1) then exit;
  ...
  if (AIndex = GetFirstMainIndex) then
  begin
    FTimeLineMin := MaxInt;
    FTimeLineMax := -MaxInt;
  end;
...
//  wmtlPlaylist.DataGroup.Visible := False;
  ...
  try
    ProgItem := GetProgramItemByIndex(AIndex);
    if (ProgItem <> nil) then
      AIndex := GetCueSheetIndexByItem(ProgItem);
  ...
  SideFrames := TimecodeToFrame(GetPlusTimecode(GV_SettingTimeParameter.AutoIncreaseDurationBefore, GV_SettingTimeParameter.AutoIncreaseDurationAmount)) - 1;
  with wmtlPlaylist do
  begin
...
//  wmtlPlaylist.MarkOut := FTimeLineMax - 1000;
  ...
  if (FChannelTimelineForm <> nil) then
  begin
    FChannelTimelineForm.DisplayPlayListTimeLine(wmtlPlaylist);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FChannelTimelineForm <> nil) then
        FChannelTimelineForm.DeletePlayListTimeLine(CompIndex, AItem);
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FChannelTimelineForm <> nil) then
        FChannelTimelineForm.DeletePlayListTimeLine(CompIndex, Item);
    end;
  end;
end;
...
begin
  ...
  if (FChannelTimelineForm <> nil) then
    FChannelTimelineForm.SetChannelOnAir(AOnAir);

  MCCSetOnAirs(ChannelID, FChannelOnAir);
end;
...
var
  ...
  LoadCueSheetList: TCueSheetList;          // 로딩한 채널 큐시트 목록
...
//              if (MoveFile(PChar(LoadFileName), PChar(WorkFileName))) then
              ...
              begin
                ...
                RemoveItemList := TList<Integer>.Create;
...
//                        StartTime := Item^.StartTime;
                  ...
                  begin
                    Item := LoadCueSheetList[RemoveItemList[I]];
...
//                        acgPlaylist.RemoveChildRow(acgPlaylist.RealRowIndex(RemoveRowList[J]));
            deletePlayListGridMain(RemoveRowList[I]);
                      ...
                      for I := RemoveItemList.Count - 1 downto 0 do
                      begin
                        Item := FCueSheetList[RemoveItemList[I]];
...
//            acgPlaylist.RemoveChildRow(acgPlaylist.RealRowIndex(RemoveRowList[J]));
          ...
          begin
            Item := FCueSheetList[RemoveItemList[J]];
...
begin
  ...
  begin
    ...
    if (MCC <> nil) then
    begin
      MCC^.Opened := (MCCBeginUpdate(MCC^.ID, ChannelID) = D_OK);
      if (not (MCC^.Opened)) then exit;

      try
        MCC^.Opened := (MCCSetOnAir(MCC^.ID, ChannelID, ChannelOnAir) = D_OK);
        if (not (MCC^.Opened)) then exit;
        ...
        begin
          for I := 0 to FCueSheetList.Count - 1 do
          begin
            MCC^.Opened := (MCCInputCueSheet(MCC^.ID, I, FCueSheetList[I]^) = D_OK);
            if (not (MCC^.Opened)) then
              exit;
          end;

          if (CueSheetNext <> nil) then
          begin
            MCC^.Opened := (MCCSetCueSheetNext(MCC^.ID, CueSheetNext^.EventID) = D_OK);
            if (not (MCC^.Opened)) then exit;
          end;

          if (CueSheetCurr <> nil) then
          begin
            MCC^.Opened := (MCCSetCueSheetCurr(MCC^.ID, CueSheetCurr^.EventID) = D_OK);
            if (not (MCC^.Opened)) then exit;
          end;

          if (CueSheetTarget <> nil) then
          begin
            MCC^.Opened := (MCCSetCueSheetTarget(MCC^.ID, CueSheetTarget^.EventID) = D_OK);
            if (not (MCC^.Opened)) then exit;
          end;
        end;
      finally
        if (MCC^.Opened) then
          MCC^.Opened := (MCCEndUpdate(MCC^.ID, ChannelID) = D_OK);
      end;
    end;
  end;
end;
...
{      if (PGroupNo <> CItem^.GroupNo) then
      ...
      begin
        ...
        if (R <> D_OK) then
        begin
//          if {(FCueSheetNext = nil) or }(CueSheetNext = CItem) or
//             ((CueSheetNext <> nil) and (CueSheetNext^.GroupNo = CItem^.GroupNo)) then
          if (StartItem = CItem) or (StartItem^.GroupNo = CItem^.GroupNo) then
          begin
...
//          ShowMessage('not on air');
  ...
  //      FTimerThread.Resume;
    ...
    //        StartTime := CurrItem.StartTime;
            ...
            ResetStartTimeByTime(SkipIndex, OnAirStartTime);
          end
    ...
    //        StartTime := NextItem.StartTime;
            SkipIndex := NextIndex;
            SetCueSheetItemStatusByIndex(GetStartOnAirMainIndex, SkipIndex - 1, esSkipped);
            ResetStartTimeByTime(SkipIndex, OnNextStartTime);
          end
          else exit;
...
//    acgPlaylist.AllCells[IDX_COL_CUESHEET_START_MODE, NextIndex + CNT_CUESHEET_HEADER] := StartModeNames[CueSheetNext^.StartMode];
    ...
    MCCInputCueSheets(NextIndex, CueSheetNext);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (ChannelOnAir) then
        begin
          ResetEvent(GV_MCCCheckExecuteEvent);
          MCCBeginUpdates(ChannelID);
        end;
        try
  ...
  //        DisplayPlayListGrid(StartIndex, 0);
          ...
          if (ChannelOnAir) then
          begin
            MCCInputCueSheets(ChannelID, EndIndex);
          ...
          begin
            MCCEndUpdates(ChannelID);
            SetEvent(GV_MCCCheckExecuteEvent);
          end;
        end;
      end;
//      CueSheetNext := CItem;
...
begin
  ...
  PostMessage(Handle, WM_UPDATE_CURR_EVENT, CurrIndex, NativeInt(CueSheetCurr));
end;
...
//  if (not FChannelOnAir) then exit;
  ...
  begin
    ...
    begin
      ...
      if (ChannelOnAir) then MCCSetEventStatuses(Item^.EventID, Item^.EventStatus);
    end;
  end;
end;
...
//      Repaint;
  ...
  if (FChannelTimelineForm <> nil) then
  begin
    FChannelTimelineForm.ClearPlayListTimeLine;
  end;
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            EventStatus.ErrorCode := ERR_NoError; // 추후 errorcode 포함여부 결정
...
{            // Play list를 처음 open 했을 경우에만 큐시트의 GroupNo로 저장
    ...
    AChannelCueSheet^.EventCount := ACueSheetList.Count;
  finally
    FreeAndNil(Xml);
  end;
end;
...
begin
  ...
  begin
    ...
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCBeginUpdate(MCC^.ID, AChannelID) = D_OK);
  end;
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCEndUpdate(MCC^.ID, AChannelID) = D_OK);
  end;
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCSetOnAir(MCC.ID, AChannelID, AIsOnAir) = D_OK);
  end;
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCSetEventStatus(MCC^.ID, AEventID, AEventStatus) = D_OK);
  end;
end;

procedure TfrmChannel.MCCInputCueSheets(AChannelID: Word; AIndex: Integer);
var
  I: Integer;
begin
  if (AIndex < 0) or (AIndex > FCueSheetList.Count - 1) then exit;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCInputCueSheet(MCC^.ID, AIndex, AItem^) = D_OK);
  end;
end;

procedure TfrmChannel.MCCDeleteCueSheets(AChannelID: Word; ARemoveIndexList: TList<Integer>);
var
  I: Integer;
  MCC: PMCC;
begin
  if (ARemoveIndexList = nil) or (ARemoveIndexList.Count <= 0) then exit;

  for I := ARemoveIndexList.Count - 1 downto 0 do
    MCCDeleteCueSheets(FCueSheetList[ARemoveIndexList[I]]^.EventID);
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCDeleteCueSheet(MCC^.ID, AEventID) = D_OK);
  end;
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCSetCueSheetCurr(MCC^.ID, AEventID) = D_OK);
  end;
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCSetCueSheetNext(MCC^.ID, AEventID) = D_OK);
  end;
end;
...
begin
  ...
  begin
    MCC := GV_MCCList[I];
    if (MCC <> nil) and (MCC^.Opened) then
      MCC^.Opened := (MCCSetCueSheetTarget(MCC^.ID, AEventID) = D_OK);
  end;
end;
